<?php
/**
 * Created by JetBrains PhpStorm.
 * User: 饭
 * Date: 13-3-25
 * Time: 上午11:01
 * To change this template use File | Settings | File Templates.
 */
$lang['site_name'] ="西安菲尔特金属过滤材料有限公司";
$lang['copyright'] = '版权所有 西安菲尔特金属过滤材料有限公司 1993-2013。 保留一切权利。陕ICP备06013362号';
$lang['contact-title']='简易信息框';
$lang['contact-user']='名称';
$lang['contact-unit']='単位';
$lang['contact-email']='Eメール';
$lang['contact-phone']='電話番号';
$lang['contact-message']='情報';
$lang['main-product']='主要产品';
$lang['aboutus']='关于菲尔特';
$lang['site-links']='関連サイト';
$lang['site-biz']='ウェブサービス';
$lang['s1'] ='アプリケーション';
$lang['s11'] ='織物';
$lang['s111'] ='純金属繊維ライン';
$lang['s112'] ='金属繊維混紡糸';
$lang['s12'] ='車';
$lang['s121'] ='ディーゼル排気フィルター';
$lang['s122'] ='車のシートヒーター';
$lang['s123'] ='自動車用エアバッグ';
$lang['s13'] ='機器';
$lang['s131'] ='バーナー';
$lang['s132'] ='インクジェットプリンタ';
$lang['s14'] ='化学';
$lang['s141'] ='石炭化学';
$lang['s142'] ='肥厚';
$lang['s15'] ='化学繊維';
$lang['s16'] ='ビスコース';
$lang['s17'] ='プラスチック';
$lang['s18'] ='太陽';
$lang['s19'] ='電子';
$lang['s1110'] ='ガラス';
$lang['s1111'] ='医学';
$lang['s1112'] ='冶金';
$lang['s2'] ='製品';
$lang['s21'] ='ステンレス鋼繊維';
$lang['s211'] ='ステンレス鋼繊維';
$lang['s212'] ='アイアンブランドアルミニウム繊維';
$lang['s213'] ='ニッケル繊維';
$lang['s214'] ='ハステロイ繊維';
$lang['s215'] ='ジルコニウム繊維';
//$lang['s216'] ='压花纤维';
$lang['s22'] ='繊維マット';
$lang['s221'] ='ステンレス鋼繊維のフェルト';
$lang['s222'] ='アイアンブランドアルミニウム繊維フェルト';
$lang['s223'] ='ニードルフェルト';
$lang['s224'] ='スパンレースフェルト';
//$lang['s225'] ='水刺毡';
$lang['s23'] ='多層金属焼結網';
$lang['s231'] ='5階建ての金属焼結網';
$lang['s232'] ='6階建ての金属焼結網';
$lang['s24'] ='金属繊維製品';
$lang['s241'] ='金属繊維は、フィルタが生成感じ';
$lang['s242'] ='多層金属焼結金網の生産';
$lang['s243'] ='フィルターディスク';
$lang['s244'] ='高温ガスダストフィルターバッグ';
$lang['s245'] ='太陽蓄熱ネットワークコア';
$lang['s246'] ='多層メッシュフィルター';
$lang['s247'] ='組紐と燃焼';
$lang['s3'] ='技術の研究開発';
$lang['s31'] ='R＆Dプラットフォーム';
$lang['s311'] ='多孔質材料の国立研究所';
$lang['s312'] ='地方自治体の技術センター';
//$lang['s313'] ='市営企業技術センター';
$lang['s32'] ='技術を持っている';
$lang['s321'] ='金属繊維技術';
$lang['s322'] ='金属繊維マット技術';
$lang['s323'] ='焼結金属メッシュテクノロジー';
$lang['s324'] ='様々なフィルタの設計と加工技術';
$lang['s33'] ='R＆Dの結果';
$lang['s331'] ='プロジェクト';
$lang['s332'] ='特許';
$lang['s333'] ='標準';
$lang['s4'] ='品質管理';
$lang['s41'] ='品質方針';
$lang['s42'] ='品質管理システム';
$lang['s43'] ='検出システムの厳密な分析';
$lang['s44'] ='厳密なプロセス制御';
$lang['s5'] ='公司概况';
$lang['s51'] ='会社の概要';
$lang['s52'] ='開発コース';
$lang['s53'] ='企業文化';
$lang['s54'] ='名誉';
$lang['s55'] ='リーディングケア';
$lang['s56'] ='社会的責任（CSR）';
$lang['s561'] ='省エネと環境保護';
$lang['s562'] ='健康と安全';
$lang['s57'] ='会社案内パンフレット';
$lang['s6'] ='お問い合わせ';
$lang['s61'] ='私たちについて';
$lang['s62'] ='サイトマップ';
$lang['s63'] ='よくある質問';

$lang['home-a-name']='R＆D';
$lang['home-a-html']='<div class="yf_dx"></div>
        <div class="yf_01">
            <div class="yf_img"><img src="/resources/images/common/yfimg01.jpg" alt="R＆D" /></div>
            <h1><a href="/welcome/catalog/311">多孔質材料の国立研究所</a></h1>
            <h2>金属繊維の調製技術、金属繊維多孔質材料の準備理論材料特性と理論研究のアプリケーションを実行するための研究所は、
            同社のR＆D機能を強化するための強固な基盤を築きました。</h2>
        </div>
        <div class="yf_01">
            <div class="yf_img"><img src="/resources/images/common/yfimg02.jpg" alt="R＆D" /></div>
            <h1><a href="/welcome/catalog/312">市営企業技術センター</a></h1>
            <h2>製品開発における技術センター、経営革新、技術革新、産業オートメーション、省エネ•排出削減、
            独自のコア独自の技術の形成は、企業自体に積極的な役割だけでなく、産業の急速な発展を果たしてきま。</h2>
        </div>';



$lang['home-b-name']='会社の概要';
$lang['home-b-html']=' <div class="gs_dx"></div>
        <div class="gs_info">
            <h1>フュルト金属フィルター材料有限公司の西安は、金属繊維およびその製品
            のR＆Dと製造企業におけるグローバルリーダーです。主な製品は金属繊維であ
            り、金属繊維のフェルト、焼結金属、金属......</h1>
        </div>
        <div class="gs_info02">
            <ul>
                <li>1993年に設立され</li>
                <li>同国最大の金属繊維およびその製品の生産拠点。</li>
                <li>金属繊維の300トンの年間生産量</li>
                <li>15万平方メートル年は焼結フィルター</li>
                <li>ナショナル多孔性材料研究室サポートユニット</li>
                <li>クマ15国と地方の主要プロジェクト</li>
                <li>9認可された特許</li>
                <li>西部材の保持子会社（深セン証券取引所上場企業、証券コード002149）</li>
            </ul>
        </div>';


$lang['home-c-name']='連絡';
$lang['home-c-html']='<div class="call_dx"></div>
        <div class="call_info">
            <h1>住所：中国西安経済技術開発区、兆銘工業団地ウエストゴールデンロード15</h1>
            <h1>郵便番号：710201</h1>
            <h1>電話:0086-29-86968425  0086-29-86968420</h1>
            <h1>ファックス:0086-29-86968425  0086-29-86968420</h1>
            <h1>E-MAIL：xianfilter@yahoo.com.cn（国内）xianfilter@gmail.com（国際）</h1>
            <h1>ウェブサイト：www.c-frt.com</h1>
        </div>';

$lang['home-d-name']='特産品';
$lang['home-d-html']='<div class="cp_dx"></div>
        <div class="cpimg">
            <ul>
                <li><img src="/resources/images/common/cpimg01.jpg" alt="特産品" /></li>
                <li><img src="/resources/images/common/cpimg02.jpg" alt="特産品" /></li>
                <li><img src="/resources/images/common/cpimg03.jpg" alt="特産品" /></li>
            </ul>
        </div>
        <div class="cpinfo">
            <ul>

                <li><a target="_blank" href="/welcome/catalog/221">ステンレス鋼繊維のフェルト</a></li>
                <li><a target="_blank" href="/welcome/catalog/222">アイアンブランドアルミニウム繊維フェルト</a></li>
                <li><a target="_blank" href="/welcome/catalog/223">ニードルフェルト</a></li>
                <li><a target="_blank" href="/welcome/catalog/224">スパンレースフェルト</a></li>
               <!-- <li><a target="_blank" href="/welcome/catalog/225">水刺毡</a></li>-->
                <li><a target="_blank" href="/welcome/catalog/211">ステンレス鋼繊維</a></li>
                <li><a target="_blank" href="/welcome/catalog/212">アイアンブランドアルミニウム繊維</a></li>


            </ul>
        </div>';



/**

るために高い幾何学的表面積を有するアルミニウム繊維マットかなりの利益、微粒子トラップ装置
（DPF）を持つ自動車の排気ガス浄化の生産のための優れた材料。
 *
 *
よってとても小さい金属繊維の小さなギャップだけでなく、良好な熱伝達特性、
その燃焼熱から流れる空気の混合気の流れ、 - 金属繊維は均一な気流、
ガスであってもよい、燃焼室内の局所高温を避ける。

 *
 *
 * が、防錆性能、（1000℃までの温度に使用）
、良好な高温があり、煤が良い持っているフィルタリングの有効性と効率性、
優れた再生能力、長寿命、省エネ効果が顕著である.
 */
$lang['home-e-name']='注目のアプリケーション';
$lang['home-e-html']=' <div class="yy_dx"></div>
        <div class="yy_info">
            <h1><a href="/welcome/catalog/121">自動車排ガス浄化キャリア</a></h1>
            <div class="yy_info02">
                <div class="yy_info_img"><img src="/resources/images/common/yyimg01.jpg"
                 alt="注目のアプリケーション" /></div>
                <h2>アイアンブランド触媒吸着、高気孔率、およびスルーホールの度合燃焼ガスの全額、
                熱伝導率の高いキャリア急速加熱性能......</h2>
            </div>
        </div>


        <div class="yy_info">
            <h1><a href="/welcome/catalog/131">バーナー</a></h1>
            <div class="yy_info02">
                <div class="yy_info_img"><img src="/resources/images/common/yyimg02.jpg" alt="注目のアプリケーション" /></div>
                <h2>バーナーヘッドは、金属繊維編組、金属繊維編組を通って燃焼室で燃焼ガスの層が配置されている。
                効果的に燃焼室......</h2>
            </div>
        </div>
        <div class="yy_info">
            <h1><a href="/welcome/catalog/1112">高温ガス粉塵</a></h1>
            <div class="yy_info02">
                <div class="yy_info_img"><img src="/resources/images/common/yyimg03.jpg" alt="注目のアプリケーション" /></div>
                <h2>繊維マットの鉄のブランドアルミろ過効率、高気孔率、低抵抗は、成形プロセスを容易にするために、
                十分な強度と靭性を持っている</h2>
            </div>
        </div>';



$lang['home-f-name']='品質管理';
$lang['home-f-html']='<div class="zl_dx"></div>
        <div class="zl_info">
            <ul>
                <li><a href="/welcome/catalog/41">品質方針</a></li>
                <li><a href="/welcome/catalog/42">品質管理システム</a></li>
                <li><a href="/welcome/catalog/43">検出システムの厳密な分析</a></li>
                <li><a href="/welcome/catalog/44">厳密なプロセス制御</a></li>
            </ul>
        </div>';



$lang['home-g-name']='関連リンク';
$lang['home-g-html']=' <div class="zl_dx"></div>
        <div class="zl_info">
            <ul>
                <li><a href="http://www.c-wmm.com">www.c-wmm.com</a></li>
                <li><a href="http://www.c-nin.com">www.c-nin.com</a></li>
                <li><a href="http://www.c-wst.com">www.c-wst.com</a></li>
                <li><a href="http://www.c-tlc.com">www.c-tlc.com</a></li>

            </ul>
        </div>';


$lang['contact-details']='<ul>
            <li>住所：中国西安経済技術開発区、兆銘工業団地ウエストゴールデンロード15</li>
            <li>郵便番号：710201</li>
            <li>電話029 -86968425   029-86968420</li>
            <li>ファックス029-86968425 029-86968420</li>
            <li>E-MAIL：xianfilter@yahoo.com.cn（国内）</li>
            <li>xianfilter@gmail.com（国際）</li>
			<li>ウェブサイト：www.c-frt.com</li>


        </ul>';


$lang['style-local']='ja';
$lang['c-wmm']='Western Metal Materials Co., LTD.';
$lang['c-wmm-url']='http://www.c-wmm.com';
$lang['c-nin']='Northwest Institute for Nonferrous Metal Research';
$lang['c-nin-url']='http://www.c-nin.com';
$lang['c-wtt']='Western Titanium Technologies Co., LTD.';
$lang['c-wtt-url']='http://www.c-wtt.com';
$lang['c-tlc']='Xi’an Tianli Clad Metal Materials Co., LTD.';
$lang['c-tlc-url']='http://www.c-tlc.com';
$lang['c-rmm']='Xi’an Noble Rare Metal Materials Co., LTD.';
$lang['c-rmm-url']='http://www.c-rmm.com';
$lang['c-nnn']=' Xi’an Refra Tungsten & Molybdenum Co., LTD';
$lang['c-nnn-url']=' Xi’an Refra Tungsten & Molybdenum Co., LTD';